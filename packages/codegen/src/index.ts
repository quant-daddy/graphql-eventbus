...
export const plugin = (
  ...
  if (config.publisher) {
    ...
    if (!queryType) {
      throw new Error(
        "You must have Query type to generate publisher code"
      );
    }
    let publisherString: string = Object.entries(
      queryType.getFields()
    )
  ...
  if (config.consumer) {
    if (config.consumer.eventSampler) {
      ...
      let eventSamplerString: string = Object.entries(
        queryType.getFields()
      )
        .map(([queryName, queryReturnObject]) => {
    ...
    const returnTypes = queries.map((a: OperationDefinitionNode) => {
      ...
      if (!a.name?.value) {
        throw new Error(
          "You must specify the name for each event query"
        );
      }
